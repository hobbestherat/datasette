...
function drawBuffer(data: Float32Array, offset: number) {
...
function samplesToHighLowIntervals(data: Float32Array, sampleInterval: number) {
  let last = 0;
  let current = 0;
...
function classify(intervals: number[]) {
  ...
  function inRange(range: number[], value: number): boolean {
...
function decode(classified: number[]) {
  let i = 0;
  let parityError = 0;
  let result: number[] = [];
...
function calcHistogram(data: number[], min: number, max: number, bucketSize: number) {
  let histogram = new Array<number>(Math.floor((max - min) / bucketSize + 1)).fill(0);
